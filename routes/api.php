...
Route::get('/cities/{city_id}', function ($city_id) {
	$city = (new City)->fetch($city_id);
   	if(!$city) return JSend::fail("Can't find any city with the id $city_id");
...
Route::get('/groups/{group_id}', function($group_id) {
	...
	if(!$group) return JSend::fail("Can't find any group with ID $group_id");
...
Route::get('/centers/{center_id}', function($center_id) {
	...
	if(!$center) return JSend::fail("Can't find any center with ID $center_id");
...
Route::get('/batches/{batch_id}', function($batch_id) {
	...
	if(!$batch) return JSend::fail("Can't find any batch with ID $batch_id");
...
Route::get('/levels/{level_id}', function($level_id) {
	...
	if(!$level) return JSend::fail("Can't find any level with ID $level_id", []);
...
if(!function_exists('getData')) { // It was causing some wierd issues in 'php artisan config:cache' command.
	function getData($item, $item_id, $data_name) {
		...
		if(!$data) return JSend::fail("Can't find any Data with $item ID $item_id", []);
...
Route::post('/users/{user_id}/groups/{group_id}', function($user_id, $group_id) {
	...
	if(!$groups) return JSend::fail("User already has the given group", [], 400);
...
Route::delete('/users/{user_id}/groups/{group_id}', function($user_id, $group_id) {
	...
	if(!$groups) return JSend::fail("User don't have the given group", [], 400);
...
Route::delete('/students/{student_id}', function($student_id) {
	...
	if(!$info) return JSend::fail("Can't find student with id '$student_id'");
...
Route::get('/students/{student_id}', function($student_id) {
	...
	if(!$details) return JSend::fail("Can't find student with id '$student_id'");
...
Route::post('/deposits/{deposit_id}', function ($deposit_id, Request $request) {
	...
	if(!$given_deposit) return JSend::fail("Can't find any deposit with the given id.", $deposit->errors);
	...
	} else return JSend::error("Status should be 'approved' or 'rejected'", [], 400);
...
Route::get('/events/{event_id}', function($event_id) {
	...
	if(!$data) return JSend::fail("Can't find event with ID $event_id", $event->errors);
...
Route::post('/events/{event_id}/users', function($event_id, Request $request) {
	...
	if(!$event) return JSend::fail("Can't find event with ID $event_id", $event->errors);
...
Route::get('/events/{event_id}/users/{user_id}', function($event_id, $user_id) {
	...
	if(!count($data)) return JSend::fail("Can't find event with ID $event_id / User with ID $user_id", $event->errors);
