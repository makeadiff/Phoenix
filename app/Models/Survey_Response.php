...
final class Survey_Response extends Model
{
    ...
    public static function search($data)
    {
        ...
        if (!empty($data['response_id'])) {
            $q->where('id', $data['response_id']);
        }
        if (!empty($data['survey_id'])) {
            $q->where('survey_id', $data['survey_id']);
        }
        if (!empty($data['question_id'])) {
            $q->where('survey_question_id', $data['question_id']);
        }
        if (!empty($data['responder_id'])) {
            $q->where('responder_id', $data['responder_id']);
        }
    ...
    {
        if (empty($fields['survey_id']) and $survey_id) {
