...
final class Survey_Question extends Common
{
    ...
    public function addMany($data, $survey_template_id = 0)
    {
        ...
        foreach ($data as $index => $fields) {
            ...
            $validator = Validator::make($fields, [
                ...
                'response_type'         => 'required|in:text,choice,number,1-10,1-5,yes-no,date,datetime,file,longtext,multi-choice'
            ]);
            if ($validator->fails()) {
    ...
    /// This will create a smaller name for the question - this can be used as a smaller key/identifier for the question when exporting to CSV.
    private static function questionName($question)
    {
        ...
        $name = preg_replace(['/Make A Difference/i', '/\([^\)]+\)/', '/[\?\'\.\-\,\"\/]/', '/\s*\b$/', '/\W$/'], ['MAD', '', ' ','', ''], $question);
