...
final class Survey extends Model
{
    ...
    public static function search($data)
    {
        ...
        $q->select(
            ...
            'Survey_Template.vertical_id',
            'Survey_Template.options',
            'Survey.survey_template_id',
            app('db')->raw('Survey_Template.name AS template_name')
        );
        $q->join('Survey_Template', 'Survey_Template.id', '=', 'Survey.survey_template_id');

        if (!empty($data['id'])) {
            $q->where('Survey.id', $data['id']);
        }
        if (!empty($data['survey_id'])) {
            $q->where('Survey.id', $data['survey_id']);
        }
        if (!empty($data['name'])) {
            $q->where('Survey_Template.name', 'like', '%' . $data['name'] . '%');
        }
        if (!empty($data['description'])) {
            $q->where('Survey_Template.description', 'like', '%' . $data['description'] . '%');
        }
        if (!empty($data['vertical_id'])) {
            $q->where('Survey_Template.vertical_id', $data['vertical_id']);
        }
        if (!empty($data['responder'])) {
            $q->where('Survey_Template.responder', $data['responder']);
        }
        if (!empty($data['survey_template_id'])) {
            $q->where('Survey.survey_template_id', $data['survey_template_id']);
        }
