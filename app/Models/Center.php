...
final class Center extends Common
{
    ...
    {
        return $this->hasMany('App\Models\Batch')->where('Batch.status','1')->where('Batch.year', $this->year)->where('Batch.project_id', $project_id);
    }
    public function levels($project_id = 1)
    {
        return $this->hasMany('App\Models\Level')->where('Level.status','1')->where('Level.year',$this->year)->where('Level.project_id', $project_id);
    }
