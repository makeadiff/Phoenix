...
final class Notification extends Common
{
    ...
    public function search($data)
    {
        ...
        if ($data['status'] !== false) {
            $q->where('Push_Notification.status', $data['status']);
        } // Setting status as '0' gets you even the disabled ones.

        if (!empty($data['user_id'])) {
            $q->where('Push_Notification.user_id', $data['user_id']);
        }
        if (!empty($data['imei'])) {
            $q->where('Push_Notification.imei_no', $data['imei']);
        }
        if (!empty($data['fcm_regid'])) {
            $q->where('Push_Notification.fcm_regid', $data['fcm_regid']);
        }
        if (!empty($data['platform'])) {
            $q->where('Push_Notification.platform', $data['platform']);
        }
        if (!empty($data['app'])) {
            $q->where('Push_Notification.app', $data['app']);
        }
        if (!empty($data['phone'])) {
